...
public class calendario extends JFrame implements ActionListener {
	...
	private JButton anterior, siguiente, ir, recordatorio, guardar, cancelar, importar;
	private DefaultTableModel tabla;
	private JTable table;
	private JFrame marco;
	private JTextArea actividadT;

	String dias[] = {"Lunes","Martes","MiÈrcoles","Jueves","Viernes","S·bado","Domingo"};
	...
	public boolean bisiesto(int a) { // Retorna true si el aÒo es bisiesto, de lo contrario retorna false
	...
	public void irA() { // Muestra la interfaz para la opcion "Ir a..."
		...
		JLabel seleccion = new JLabel("Seleccione una opciÛn: ");
	...
	public void escogerMes() { // Establece la interfaz y muestra en la tabla el mes seleccionado por el usuario
		...
		try {
			...
			JLabel titulo1 = new JLabel("Digite el aÒo y escoja el mes");
			...
			JOptionPane.showMessageDialog(null,e3,"SelecciÛn",JOptionPane.QUESTION_MESSAGE);
			...
			"Debe digitar numeros enteros positivos en el espacio para el aÒo","Error",
	...
	public void opcionRecordatorio() { // Muestra la interfaz para la opcion "Recordatorio"
		...
		if(columnaSeleccionada != -1 && filaSeleccionada != -1) {
			...
			JOptionPane.showMessageDialog(null,
			"Debe seleccionar una celda que no estÈ vacÌa","Error",
			JOptionPane.WARNING_MESSAGE);

			else {

				JLabel pregunta = new JLabel("øQuÈ desea hacer?");
				...
				JOptionPane.showMessageDialog(null,o1,"SelecciÛn",JOptionPane.QUESTION_MESSAGE);
...
//			c1.add(horaT);
	...
	public void guardarRecordatorio() { // Guarda en archivo lo que se escribio en los espacios de Lugar y Actividad
...
try {
			...
			if(espaciosLlenos()) {
				...
				guardarCantidad();
                                ...
                                String ical= "";
                                String filenameical = String.valueOf("recordatorio"+cantidadIcal+".ics");
                                String filenameicalzip = String.valueOf("recordatorio"+cantidadIcal+".zip");

				cantidadIcal++;

                                // Parte importante
                                ical +="BEGIN:VEVENT\n";
                                ical+= "DTSTART;VALUE=DATE:";
				ical += String.valueOf(anoTemporal);
                                ical += String.valueOf(mesTemporal+1);                                
                                ical += identificarDato(datoSeleccionado)+ "\n";
                                ical+= "SUMARY:";
                                ical += lugarT.getText() + "\n";
                                ical+= actividadT.getText() +"\n";
                                ical+="END:VEVENT\n";
                                
				File archivo = new File(filenameical);
				...
				FileOutputStream dest = new FileOutputStream(filenameicalzip);

				ZipOutputStream out = new ZipOutputStream(new BufferedOutputStream(dest));

				byte data[] = new byte[buffer];

				File f = new File(filenameical);

				FileInputStream fi = new FileInputStream(f);

				origin = new BufferedInputStream(fi, buffer);

				ZipEntry entry = new ZipEntry(filenameical);
				...
				JOptionPane.showMessageDialog(null,"El evento Ical ha sido guardado con exito",
				"Ical Guardado",JOptionPane.INFORMATION_MESSAGE);
	...
	public void verRecordatorio() { // Lee desde los archivos con recordatorios y extrae su informacion
		...
		try {
			...
			"para el "+identificarDato(datoSeleccionado)+" de "+meses[mesTemporal].toLowerCase()+" del aÒo "+anoTemporal,
